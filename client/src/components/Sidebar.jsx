...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div
        className={`flex flex-col h-screen min-w-72 px-4 py-3
        bg-[#F8FAFF] dark:bg-[#0B1020]
        border-r border-gray-200 dark:border-[#80609F]/20
        transition-all duration-500
        max-md:absolute left-0 z-50
    ...
    <div className="flex items-center justify-center w-full pt-1 pb-2">
        ...
        alt="Lucy Logo"
        className="w-44 h-12 object-contain cursor-pointer transition-all duration-300"
        />
    </div>

      {/* New Chat Button */}
      <button onClick={createNewChat} className='flex justify-center items-center w-full py-2.5 mt-4 text-white  bg-gradient-to-r from-[#A456F7] to-[#3D81F6] text-sm rounded-md cursor-pointer'>
    ...
    <div onClick={()=> {navigate('/credits'); setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
            ...
            <p className='text-xs text-gray-400'>Purchase credits to use Lucy AI</p>
        </div>
    </div>
